        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="technicianSalaryMonthly">Lương KTV/tháng (VNĐ)</Label>
                <Input 
              ...
              <div className="space-y-2">
                <Label htmlFor="numTechnicians">Số KTV thực tế muốn thuê (Loại 2)</Label>
                <Input 
                  id="numTechnicians" 
                  type="number" 
                  ...
                  <div className={`rounded-lg border p-4 mt-2 ${exceeded ? "border-red-300 bg-red-50 dark:border-red-800 dark:bg-red-950/20" : "border-primary/20 bg-primary/5"}`}>
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`text-sm font-semibold ${exceeded ? "text-red-700 dark:text-red-400" : "text-primary"}`}>
                        {exceeded ? "⚠ Quỹ lương KTV Loại 2 — Vượt trần!" : "✓ Quỹ lương KTV Loại 2 — Trong giới hạn"}
                      </span>
                    </div>
                    <p className="text-xs text-muted-foreground mb-3">Lương GĐ & Kế toán tính riêng ngoài quỹ lương này</p>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-center">
                      <div className="rounded-md bg-background border border-border p-3">
                        <div className="text-xs text-muted-foreground mb-1">Trần quỹ lương KTV ({(maxWageFundRate * 100).toFixed(0)}% DT)</div>
                      ...
                      <div className="rounded-md bg-background border border-border p-3">
                        <div className="text-xs text-muted-foreground mb-1">Chi phí KTV thực tế ({input.numTechnicians} người, có BH)</div>
                      ...
                      <div className={`rounded-md border p-3 ${exceeded ? "bg-red-50 border-red-200 dark:bg-red-950/30 dark:border-red-700" : "bg-green-50 border-green-200 dark:bg-green-950/30 dark:border-green-700"}`}>
                        <div className="text-xs text-muted-foreground mb-1">KTV tối đa có thể thuê</div>
                    ...
                    {exceeded && (
                      <div className="mt-3 text-xs text-red-700 dark:text-red-400">
                        Chi phí KTV ({formatVND(actualTechCost)}/năm) vượt trần <strong>{formatVND(actualTechCost - wageFundLimit)}</strong>. Tối đa cho phép: <strong>{maxKTV} KTV</strong>.
                      </div>
                    )}
                  </div>
                );
